...
const DAILY_WINDOW_CRON = "0,30 0,9 * * *";
const DAILY_SUMMARY_CRON = "30 11 * * *";
...
const SUMMARY_EVENING_TRIGGER = "cloudflare-summary-evening";
...
export async function runScheduledTrigger(controller, env, request = fetch) {
  ...
  if (controller.cron === DAILY_SUMMARY_CRON && hour === 11 && minute === 30) {
    return dispatchWorkflow(env, SUMMARY_EVENING_TRIGGER, request, SUMMARY_DISPATCH_URL);
  }
