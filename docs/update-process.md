...
线上定时器采用 Cloudflare Workers Cron，配置位于 `external-scheduler/cloudflare/wrangler.jsonc`。信息流主计划 `0 0 * * *`（UTC）对应每日北京时间 `08:00`；信息流补偿计划 `30 0 * * *`（UTC）对应北京时间 `08:30`。每日摘要主计划 `0 11 * * *`（UTC）对应每日北京时间 `19:00`；每日摘要补偿计划 `30 11 * * *`（UTC）对应北京时间 `19:30`。补偿计划会读取当天 GitHub Actions 运行记录，只有已发现成功完成的对应主触发运行时才跳过补偿；主运行缺失或失败时会再次调用对应工作流，不依赖本机开机状态或本机时间任务。
