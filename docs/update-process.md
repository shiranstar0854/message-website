...
阶段 4 已实现每日自动更新。工作流文件为 `.github/workflows/daily-update.yml`，每天北京时间 `08:17`（UTC `00:17`）运行；若 GitHub 调度丢弃该任务，`08:47`（UTC `00:47`）会再尝试一次。当天归档已生成时，备用任务会跳过重复更新。工作流也支持在 GitHub Actions 页面手动触发。
...
- 计划任务每天有一次备用触发；如果主要触发已经生成当天归档，备用触发不会重复发布。
