...
- 今日核心热点 Top 5：按评分和发布时间选出最值得先看的 5 条信息，展示中文摘要、来源、频道、发布时间和评分。
- 频道概览：用紧凑横向条显示科技、金融、新闻信息数量，不占用首屏头部空间。
- 顶部关键词搜索：匹配标题、摘要、正文摘录、中文摘要、来源、标签和关键词；多个关键词用空格分隔时默认全部命中。
- 来源状态：显示活跃来源、异常来源、最近检查和最近成功时间。

评分综合来源可信度、关键词命中、发布时间新鲜度、重复/聚合信号、频道权重和官方来源加权。评分只用于排序，不代表投资建议或事实核验结论。

英文来源会优先展示中文摘要；原英文标题仍保留为原文链接，便于回到来源核对。
...
历史页保留最近 10 天每日归档。选择日期后可查看当天信息条目。
...
- `data/archive/daily/`：每日归档，保留最近 10 天
